...
module.exports = async (m, Obj) => {
  ...
  // ===== LOGGER + AUTO BACKUP SYSTEM (FIXED) =====
  const testResults = []
  ...
  // ===== AUTO BACKUP HANDLER (FIXED - pakai path module biar aman) =====
  const backupHandlers = async () => {
    ...
    // Buat folder backup (aman)
    const backupDir = path.join(process.cwd(), "backup")
    if (!fs.existsSync(backupDir)) {
      fs.mkdirSync(backupDir, { recursive: true })
    }

    // Path yang benar (FIXED)
    const timestamp = Date.now()
    const backupPath = path.join(backupDir, `button-test-full-backup-${timestamp}.json`)
  ...
  // ===== START =====
  await conn.sendMessage(m.chat, {
    text: "🧪 START ULTRA TEST SYSTEM v2.1 (Modern Interactive Button + Fixed Backup)"
  }, { quoted: q('fkontak') })
  ...
  // =========================
  // 🔹 BUTTON BASIC (Modern itsukichan)
  // =========================
  ...
  // =========================
  // 🔹 AUTO BACKUP + FINAL (dipindah ke sini biar tetap jalan meski ada error sebelumnya)
  // =========================
...
module.exports.command = ["testsys", "testfull", "debugui", "testbutton"]