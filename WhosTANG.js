...
const { 
    ...
    getReplyModeStats,
    initReplyMode 
} = require('./Library/replyMode');

// Initialize reply mode
initReplyMode();
...
// Global state untuk reply mode per user
global.userReplyModes = {};

// Set untuk tracking user yang sedang memilih reply mode
...
/**
 * =========================================
 * 📌 FUNCTION: WhosTANG (JANGAN DIUBAH)
 * -----------------------------------------
 * Deskripsi:
 * Handler utama untuk semua pesan masuk
 * Mengelola routing command dan eksekusi fitur
 *
 * Parameter:
 * - WhosTANG → WhatsApp socket instance
 * - m → Serialized message object
 * - chatUpdate → Chat update event object
 * - store → In-memory store
 *
 * ⚠️ NOTE:
 * Ini adalah core function utama bot
 * =========================================
 */
module.exports = WhosTANG = async (WhosTANG, m, chatUpdate, store) => {
    try {
        ...
        // Tentukan body berdasarkan tipe pesan
        ...
        /**
         * Send reply mode selection to new user
         */
...
Pilih sesuai preferensi kamu ya! 😊`;
        ...
        /**
         * Handle reply mode button response
         */
        ...
        /**
         * Get user's reply mode (with fallback)
         */
        ...
        if (buttonResponse) {
            // Check if this is a reply mode selection
            if (await handleReplyModeResponse(buttonResponse, m.sender)) {
                return; // Stop processing if it was a reply mode response
            }
        }
        ...
        if (isCmd && isNewUser(m.sender) && !global.pendingReplyModeSelection.has(m.sender)) {
            // Don't ask for menu command or basic commands
            ...
            if (!skipCommands.includes(command)) {
                ...
                return; // Stop processing until user selects mode
            }
        }
        ...
        // =========================================
        /**
         * Convert ESM to CJS using Babel
         */
...
`;
            ...
            case "esm2cjs": {
                ...
                try {
                    ...
                    const useBabel = false; // ganti ke true kalau mau pakai Babel
                    let converted;
